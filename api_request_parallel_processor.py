...
async def process_api_requests_from_file(
    ...
    file = open(requests_filepath)

    # `requests` will provide requests one at a time
    requests = file.__iter__()
    if metadata_filepath is not None:
        metadata = metadata_file.__iter__()
    else:
        metadata = None
        ...
        if next_request is None:
            if queue_of_requests_to_retry.empty() is False:
                ...
                try:# get new request
                    ...
                    if metadata_filepath is not None:
                        metadata_json = eval(next(metadata))
                    else:
                        metadata_json = None
    ...
    async def call_API(
        ...
        if error:
            ...
            if self.attempts_left:
                ...
                append_to_jsonl([self.request_json, self.result, self.metadata], save_filepath)
                status_tracker.num_tasks_in_progress -= 1
                status_tracker.num_tasks_failed += 1
        else:
            append_to_jsonl([self.request_json, response, self.metadata], save_filepath)
            status_tracker.num_tasks_in_progress -= 1
    ...
    if api_endpoint == "completions":
        if "prompt" not in request_json:
            if "messages" in request_json:
                ...
                raise ValueError(
                    'Expecting either "prompt" or "messages" field in completion request'
                )
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--metadata_filepath", default=None)
    parser.add_argument("--request_url", default="https://api.openai.com/v1/embeddings")
    parser.add_argument("--api_key", default=os.getenv("OPENAI_API_KEY"))
    parser.add_argument("--max_requests_per_minute", type=int, default=100)
    parser.add_argument("--max_tokens_per_minute", type=int, default=90_000)
